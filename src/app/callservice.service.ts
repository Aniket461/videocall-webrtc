...
export class CallserviceService {
  ...
  async connectToPeer(myid: string, callerid: string) {
    ...
    if (this.conn != undefined) {
      ...
      const dialogref = this.dialog.open(BasicpopupComponent, {
        data: { message: 'User is not online right now!, call cannot be placed...', isfuncPresent: false },
      });
  ...
  async basicpopup(message: string) {
    const dialogref = this.dialog.open(BasicpopupComponent, {
      data: { message: message, isfuncPresent: false },
    });
    dialogref.afterOpened().subscribe((res) => {
      setTimeout(() => {
        window.location.href = '/userlist';
  ...
  async RecieveCall() {
    this.peer.on('call', async (call: any) => {
      ...
      this.sound = new Howl({
        src: ['../assets/teams_next_level.mp3'],
        loop: true,
      });
